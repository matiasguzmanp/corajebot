...
class FindPlaceToHide:
    ...
    def __init__(self, map, origin, resolution):
        ...
        self.possible_hiding_points = None
    ...
    def WhereCanTheRobotBe(self, possible_hiding_points):
        filtered_img = convolve2d(possible_hiding_points, self.robot_footprint, mode='same', boundary='fill', fillvalue=0)
    ...
    def point_to_original_size(self, xy_hidingplace):
        original_i = xy_hidingplace[0]*1/(self.scale_percent*0.01)
        original_j = xy_hidingplace[1]*1/(self.scale_percent*0.01)

        return original_i, original_j
    
    def calculateDistances(self, valid_points_map, point):
        x,y = point
        L = np.zeros(self.scaled_map.shape)
        for i in range(self.scaled_map.shape[0]):
            for j in range(self.scaled_map.shape[1]):
                if self.scaled_map[i,j] == 255:
    ...
    def find_hiding_place(self, paparazzi, robot, display=False, range = 300):
        ...
        t0 = time()
        possible_hiding_points_raw = mark_visible_cells(self.scaled_map, paparazzi, range)
        print('Exec time: %f'%(time()-t0))
        possible_hiding_points = cv.erode(possible_hiding_points_raw, kernel=np.ones((3, 3), np.uint8))
        self.possible_hiding_points = possible_hiding_points_raw

        valid_points_map = self.WhereCanTheRobotBe(possible_hiding_points)
        ...
        pixel_x, pixel_y = self.point_to_original_size(max_indices)
        x = self.origin[0] + pixel_x*self.resolution
        y = self.origin[1] + pixel_y*self.resolution
